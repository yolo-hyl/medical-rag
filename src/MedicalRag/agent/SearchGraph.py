...
class SearchGraph:
    ...
    def _should_call_tool(self, last_ai: BaseMessage) -> bool:
        """ 判断上一步是否触发了工具 """
        return bool(getattr(last_ai, "tool_calls", None))

    
    def llm_db_search_node(self, state: SearchMessagesState) -> SearchMessagesState:
        """ DB 检索节点 可能出发db_tool """
        query = state["query"]
        db_ai = self.db_search_llm.invoke([
        ...
        if self._should_call_tool(db_ai):
            ...
            tool_msgs : ToolMessage = self.db_tool_node.invoke([db_ai])
    ...
    def llm_network_search_node(self, state: SearchMessagesState) -> SearchMessagesState:
        """ 联网检索节点  可能触发web_tool """
        if self.config.multi_dialogue_rag.console_debug:
        ...
        fixing_parser = OutputFixingParser.from_llm(parser=parser, llm=self.llm)  # 使用不绑定工具的LLM
        ...
        # 步骤1：判断是否需要搜索
        judge_chain = judge_messages | self.llm | RunnableLambda(lambda x: del_think(x.content)) | fixing_parser  # 使用不绑定工具的LLM
        ...
        if result.need_search and result.search_query.strip():
            ...
            # 创建搜索链
            search_chain = calling_messages | self.network_search_llm
            ...
            # 检查是否有工具调用
            if self._should_call_tool(search_ai):
                tool_msgs: ToolMessage = self.network_tool_node.invoke([search_ai])
                state["other_messages"].append(tool_msgs)
    ...
    # ---------- 节点：RAG ----------
    def rag_node(self, state: SearchMessagesState) -> SearchMessagesState:
        if self.config.multi_dialogue_rag.console_debug:
            logger.info(f"开始RAG...")
    ...
    # ---------- 节点：Judge，返回路由标签 ----------
    def judge_node(self, state: SearchMessagesState) -> SearchMessagesState:
        """判断节点：负责判断和修改状态"""
        if self.config.multi_dialogue_rag.console_debug:
            logger.info(f"开始评估...")
        judge_ai = self.llm.invoke([
    ...
    def judge_router(self, state: SearchMessagesState) -> str:
        """简单的路由函数：只读取状态，不修改"""
        return state.get("judge_result", "fail")

    def finish_success(self, state: SearchMessagesState) -> SearchMessagesState:
        """ 结束节点：成功输出 """
        state["final"] = (state.get("summary", "") or "").strip() or "（空）"
        return state

    def finish_fail(self, state: SearchMessagesState) -> SearchMessagesState:
        """ 结束节点：失败警告输出 """
    ...
    def build_search_graph(self):
        ...
        # 原子节点
        g.add_node("db_search", self.llm_db_search_node)
        g.add_node("web_search", self.llm_network_search_node)
        g.add_node("rag", self.rag_node)
        g.add_node("finish_success", self.finish_success)
        g.add_node("finish_fail", self.finish_fail)
